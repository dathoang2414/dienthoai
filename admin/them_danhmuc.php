...
if(isset($_POST['btnthem']))
{
	$tendm=$_POST['tendm'];
	$dequi=$_POST['dequi'];
	$insertdm = $conn->query("INSERT INTO danhmuc VALUES('', '$tendm', '$dequi') ");
	if($insertdm) {
		...
		<tr>
			...
			<td>
            	<select name="dequi">
                    ...
                    <?php
						$show = $conn->query("SELECT * FROM danhmuc WHERE dequi=0");
						while($show1 = mysqli_fetch_array($show))
						{
							$madm = $show1['madm'];	
							$tendm = $show1['tendm'];
							echo "<option value='".$madm."'>".$tendm."</option>";	
								$show2 = $conn->query("SELECT * FROM danhmuc WHERE dequi='".$madm."'");
